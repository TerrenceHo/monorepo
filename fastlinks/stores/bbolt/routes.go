...
func (rs *RoutesStore) Create(route *models.Route) error {
	if err := rs.db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket(rs.bucketName())
		...
		return stackerrors.Wrapf(err, "failed to create route with key %s", route.Key)
	}
	return nil
}

func (rs *RoutesStore) Update(route *models.Route) error {
	if err := rs.db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket(rs.bucketName())
		...
		return b.Put([]byte(route.Key), bytes.Bytes())
	}); err != nil {
		return stackerrors.Wrapf(err, "failed to update route with key %s", route.Key)
	}
	return nil
}
...
func (rs *RoutesStore) Delete(key string) error {
	if err := rs.db.Update(func(tx *bolt.Tx) error {
		...
		return stackerrors.Wrapf(err, "failed to update route with key %s", key)
	}
	return nil
}
